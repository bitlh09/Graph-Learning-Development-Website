...
def build_planetoid_splits(y, num_classes=None, train_per_class=20, val_size=500, test_size=1000, seed=42):
    ...
    for arr in idx_by_class:
        chosen = rng.choice(arr, train_per_class, replace=False)
        train_idx.extend(chosen.tolist())
    rest = np.setdiff1d(np.arange(len(y_np)), train_idx)
    rest = rng.permutation(rest)
    val_idx = rest[:val_size].astype(np.int64)
    test_idx = rest[val_size:val_size+test_size].astype(np.int64)
    return np.array(train_idx, dtype=np.int64), val_idx, test_idx

def normalize_adj(adj, add_self_loops=True):
    if add_self_loops:
        adj = adj + sparse.identity(adj.shape[0], dtype=np.float32, format='csr')
...
class GCN(nn.Module):
    def __init__(self, in_feats, hidden, nclass, dropout=0.5):
        super().__init__()
        self.gc1 = GCNLayer(in_feats, hidden)
        self.gc2 = GCNLayer(hidden, nclass)
...
# ---------------- training ----------------
def train_loop(model, x, adj_norm, y, idx_train, idx_val, device, epochs=200, lr=0.01, weight_decay=5e-4, patience=20, save_path='gcn_best.pth'):
    model.to(device); x = x.to(device); adj_norm = adj_norm.to(device); y = y.to(device)
    idx_train_t = torch.from_numpy(idx_train).to(device)
    idx_val_t = torch.from_numpy(idx_val).to(device)
...
def main():
    ...
    parser.add_argument('--save', type=str, default='gcn_best.pth')
    ...
    row_sum = x_np.sum(1, keepdims=True)
    ...
    adj_norm = normalize_adj(adj_sparse, add_self_loops=True)
    ...
    model = GCN(in_feats=F, hidden=args.hidden, nclass=num_classes, dropout=0.5)
    model = train_loop(model, x, adj_norm, y, idx_train, idx_val, device, epochs=args.epochs, lr=args.lr, weight_decay=args.weight_decay, patience=args.patience, save_path=args.save)
    ...
    model.eval()
    with torch.no_grad():
        logits = model(x, adj_norm.to(device))
