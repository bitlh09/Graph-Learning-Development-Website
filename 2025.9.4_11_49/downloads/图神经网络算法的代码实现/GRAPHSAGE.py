...
class SAGEConv(nn.Module):
    def __init__(self, in_feats, out_feats, bias=True):
        super().__init__()
        self.lin = nn.Linear(in_feats * 2, out_feats, bias=bias)
...
class GraphSAGE(nn.Module):
    def __init__(self, in_feats, hidden, nclass, num_layers=2, dropout=0.5):
        super().__init__()
        assert num_layers >= 2
        self.layers = nn.ModuleList()
        self.layers.append(SAGEConv(in_feats, hidden))
        for _ in range(num_layers - 2): self.layers.append(SAGEConv(hidden, hidden))
        self.layers.append(SAGEConv(hidden, nclass))
...
# ---------------- training ----------------
def train_loop(model, x, adj_dense, y, idx_train, idx_val, device, epochs=200, lr=0.01, weight_decay=5e-4, patience=20, save_path='graphsage_best.pth'):
...
def main():
    ...
    parser.add_argument('--save', type=str, default='graphsage_best.pth')
    ...
    model = GraphSAGE(in_feats=F, hidden=args.hidden, nclass=num_classes, num_layers=args.num_layers, dropout=0.5)
