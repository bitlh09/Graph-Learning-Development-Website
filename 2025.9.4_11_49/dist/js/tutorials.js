...
(function() {
  ...
  // 1) 侧边目录：手风琴（滑动下拉）
  function initAccordionTOC() {
    // 为每个 tutorial-category 结构添加“可折叠”
    $$('.tutorial-category').forEach(cat => {
      const title = $('h4', cat);
      const list = $('ul', cat);
      if (!title || !list) return;

      // 包一层以便滑动动画
      list.classList.add('accordion', 'collapsed');
      // 给标题增加可点击样式与箭头
      title.classList.add('toc-toggle', 'flex', 'items-center', 'justify-between', 'py-1');
      if (!$('.caret', title)) {
        const caret = document.createElement('span');
        caret.className = 'caret inline-block text-gray-500 ml-2';
        caret.textContent = '›';  // 右箭头
        title.appendChild(caret);
      }

      // 默认展开第一组
      if (cat === $$('.tutorial-category')[0]) {
      ...
      title.addEventListener('click', () => {
        ...
        // 折叠其它（可选，如果想互斥展开）
        // $$('.tutorial-category .accordion').forEach(x => {
        //   if (x !== list) x.classList.remove('expanded'), x.classList.add('collapsed');
        // });
        // $$('.tutorial-category .toc-toggle').forEach(t => t.classList.remove('active'));
  ...
  // 2) 锚点跳转：与 showTutorial 配合
  function initAnchors() {
    // 所有目录链接：点击时既调用 showTutorial，又设置 hash
    $$('.tutorial-link').forEach(a => {
      a.addEventListener('click', (e) => {
        ...
        if (typeof window.showTutorial === 'function') {
          window.showTutorial(id);
        }
        // 再更新地址栏并滚动
        // 这里不阻止默认行为，让浏览器处理滚动（有 scroll-behavior: smooth）
      });
    });

    // 页面载入如果有 hash，直接跳转并显示对应内容
    if (location.hash && location.hash.length > 1) {
      const id = location.hash.slice(1);
      const target = document.getElementById(id);
      if (target && typeof window.showTutorial === 'function') {
        window.showTutorial(id);
        // 滚到内容顶部（scroll-margin-top 已处理吸顶偏移）
        setTimeout(() => target.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0);
      }
    }

    // 监听 hash 变化（例如用户手动改 hash 或返回/前进）
  ...
  function highlightActiveLink(id) {
    $$('.tutorial-link').forEach(a => a.classList.remove('text-blue-600', 'font-semibold'));
    const active = $(`.tutorial-link[href="#${id}"]`);
    if (active) active.classList.add('text-blue-600', 'font-semibold');
  }
  // 包装 showTutorial：高亮对应目录项
  const _origShow = window.showTutorial;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    initAnchors();
    initQuickJump();
    // 初始高亮（如果没有 hash，就用第一个已激活块）
    const activeDiv = document.querySelector('.tutorial-content.active');
    if (activeDiv && activeDiv.id) highlightActiveLink(activeDiv.id);
  });
})();
