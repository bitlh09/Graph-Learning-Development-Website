...
(function () {
  const STORAGE_THEME = 'gl-theme';
  const STORAGE_PROGRESS = 'gl-progress'; // 已学习章节记录（id 数组）
  ...
  function initReadProgress() {
    ...
    if (!bar) return;
    const onScroll = () => {
      const el = $('.tutorial-content.active') || $('#tutorial-content');
      if (!el) return;
      const rect = el.getBoundingClientRect();
      const total = el.scrollHeight - window.innerHeight;
      const passed = Math.min(Math.max(window.scrollY - (el.offsetTop - 64), 0), total);
  ...
  function initTocFilter() {
    const input = $('#toc-filter');
    if (!input) return;
    const allLinks = $$('.tutorial-link');

    input.addEventListener('input', () => {
      const kw = input.value.trim().toLowerCase();
      allLinks.forEach(a => {
        const text = a.textContent.trim().toLowerCase();
        a.parentElement.style.display = text.includes(kw) ? '' : 'none';
      });
    });
  }

  // 4) 当前章节高亮 + 学习进度勾选
  function initActiveLinkAndProgress() {
    const allLinks = $$('.tutorial-link');
    const progress = new Set(JSON.parse(localStorage.getItem(STORAGE_PROGRESS) || '[]'));

    // 解析链接里的章节 ID（来源于 onclick="showTutorial('id')"）
    function getIdFromLink(a) {
      const on = a.getAttribute('onclick') || '';
      const m = on.match(/showTutorial\('([^']+)'\)/);
      return m ? m[1] : null;
    }
    ...
    function renderDone() {
      allLinks.forEach(a => {
        ...
        if (!id) return;
        a.classList.toggle('toc-done', progress.has(id));
      });
    }

    // 监听全局 showTutorial 调用（在 tutorials.js 里定义）
    // 我们“包一层”，在原函数完成后再执行高亮与记录
    const originalShow = window.showTutorial;
    window.showTutorial = function(id) {
      if (typeof originalShow === 'function') originalShow(id);

      // 高亮当前链接
      allLinks.forEach(a => a.classList.remove('active'));
      const active = allLinks.find(a => getIdFromLink(a) === id);
      if (active) active.classList.add('active');

      // 记录学习进度
      progress.add(id);
      localStorage.setItem(STORAGE_PROGRESS, JSON.stringify(Array.from(progress)));
      renderDone();

      // 切换章节后重建 上/下一节 导航
      buildChapterNav(id);
      // 滚到内容顶部更友好
      window.scrollTo({ top: $('#tutorial-content').offsetTop - 56, behavior: 'smooth' });
    };

    renderDone();

    // 初始高亮（欢迎页不在目录里，跳过）
    const activeDiv = $('.tutorial-content.active');
    if (activeDiv && activeDiv.id) {
      const activeLink = allLinks.find(a => getIdFromLink(a) === activeDiv.id);
  ...
  function buildChapterNav(currentId) {
    ...
    // 构建章节有序列表（按侧边栏顺序）
    const links = $$('.tutorial-link');
    const ids = links.map(a => {
      const m = (a.getAttribute('onclick') || '').match(/showTutorial\('([^']+)'\)/);
      return m ? m[1] : null;
    }).filter(Boolean);
    ...
    nav.innerHTML = `
      <button ${prevId ? '' : 'disabled'} data-id="${prevId || ''}" 
        class="px-4 py-2 rounded-lg border ${prevId ? 'hover:bg-gray-50 dark:hover:bg-gray-800' : 'opacity-50 cursor-not-allowed'}">
        ← 上一节
      </button>
      <button ${nextId ? '' : 'disabled'} data-id="${nextId || ''}" 
        class="px-4 py-2 rounded-lg bg-blue-600 text-white ${nextId ? 'hover:bg-blue-700' : 'opacity-50 cursor-not-allowed'}">
        下一节 →
      </button>
    `;
    ...
    window.onkeydown = (e) => {
      ...
      if (e.key === 'ArrowLeft' && prevId) window.showTutorial(prevId);
      if (e.key === 'ArrowRight' && nextId) window.showTutorial(nextId);
    };
  }
  ...
  window.glToast = toast; // 暴露全局

  // 7) 微博关系小测判题（你已有题目，只差这个函数）
  window.checkWeiboAnswer = function () {
    const directed = $('#weibo-directed')?.checked;
    const undirected = $('#weibo-undirected')?.checked;
    if (!directed && !undirected) return toast('请选择一个选项～');
    if (directed && !undirected) {
      toast('答对啦！微博“关注”是有向关系 ✅');
    } else {
      toast('再想想：微博关注不是必须互关哦（应为有向图）～');
    }
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    initReadProgress();
    initTocFilter();
    initActiveLinkAndProgress();

    // 首次构建章节导航（欢迎页无 id，这里用第一个 id）
    const firstLink = $$('.tutorial-link')[0];
    if (firstLink) {
      const m = (firstLink.getAttribute('onclick') || '').match(/showTutorial\('([^']+)'\)/);
      if (m) buildChapterNav(m[1]);
    }
  });
